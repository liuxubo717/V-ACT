...
from data_handling.audiocaps_video_dataset import get_audiocaps_video_loader
...
def train():
    ...
    for batch_idx, train_batch in tqdm(enumerate(training_data), total=len(training_data)):
        ...
        src, tgt, tgt_len, f_names, captions, video = train_batch
        src = src.to(device)
        tgt = tgt.to(device)
        ...
        y_hat = model(src, video, tgt, target_padding_mask=tgt_pad_mask)
...
def eval_greedy(data):
    ...
    with torch.no_grad():
        ...
        for batch_idx, eval_batch in tqdm(enumerate(data), total=len(data)):
            ...
            src, target_dicts, file_names, video = eval_batch
            src = src.to(device)
            video = video.to(device)

            output = greedy_decode(model, src, video, sos_ind=sos_ind, eos_ind=eos_ind)
...
def eval_beam(data, beam_size):
    ...
    with torch.no_grad():
        ...
        for batch_idx, eval_batch in tqdm(enumerate(data), total=len(data)):
            ...
            output = beam_decode(src, video, model, sos_ind, eos_ind, beam_width=beam_size)
...
exp_name = args.modality + '_' + args.exp_name
...
training_data = get_audiocaps_video_loader('train', config)
validation_data = get_audiocaps_video_loader('val', config)
test_data = get_audiocaps_video_loader('test', config)
