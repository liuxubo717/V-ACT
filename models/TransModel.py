...
from models.S3D import S3D, load_S3D_weight
...
class ACT(nn.Module):
    ...
    def __init__(self, config, ntoken):
        ...
        self.encoder = AudioTransformer(patch_size,
                                        ...
                                        mlp_dim,
                                        dropout=dropout)
        if config.audio_encoder.pretrained:
            pretrained_encoder = torch.load(config.path.encoder)['model']
            if config.audio_encoder.model == 'deit':
        ...
        if config.audio_encoder.freeze:
        ...
        self.video_encoder_linear = nn.Linear(1024, self.nhid)
    ...
    def encode(self, audio, video_features):
        ...
        if self.modality == 'audio':
            ...
            if self.modality == 'audio_visual':
                ...
                src_a = F.relu_(self.encoder_linear(src_a))  # batch x time x nhid
                src = torch.cat((src, src_a), dim=1)
